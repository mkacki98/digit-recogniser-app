...
def train_validate(
    model, config, train_loader, val_loader, optimizer, criterion, device, save=True
):
    ...
    writer = SummaryWriter(
        f"logs/fit/{config.model} | BatchSize{config.batch_size} | LearningRate {config.lr} | Epochs {config.epoch_n}"
    )
    ...
    with trange(config.epoch_n) as t:
        for epoch in t:
            ...
            epoch_val_loss, epoch_val_acc = epoch_validate(
                model, val_loader, criterion, writer, epoch, device, is_neumorphic
            )

            t.set_postfix(train_acc=epoch_train_acc, val_acc=epoch_val_acc)

            writer.add_scalar("epoch train loss", epoch_train_loss, epoch)
            writer.add_scalar("epoch val loss", epoch_val_loss, epoch)
            writer.add_hparams(
                {"bs": config.batch_size, "lr": config.lr},
                {"acc": epoch_val_acc, "loss": epoch_val_loss},
            )
            writer.flush()
...
def epoch_train(
    ...
    for i, data in enumerate(train_loader):
        ...
        if (i + 1) % report == 0:
            ...
            writer.add_scalar("train loss", loss, epoch * n_steps + i)
            writer.add_scalar("train accuracy", acc, epoch * n_steps + i)
...
def epoch_validate(model, val_loader, criterion, writer, epoch, device, is_neumorphic):
    ...
    for i, data in enumerate(val_loader):
        ...
        if (i + 1) % report == 0:
            ...
            writer.add_scalar("val loss", loss, epoch * n_steps + i)
            writer.add_scalar("val accuracy", acc, epoch * n_steps + i)
...
def get_pretrained_synapses(train_loader, config, from_pretrained=False, save=True):
    """
    ...
    with trange(config.hid_epoch_n) as t:
        for epoch in t:
            ...
            for i, data in enumerate(train_loader):
                ...
                input_current = torch.mm(
                    nmf_activation_fn(config.p, synapses), torch.transpose(images, 0, 1)
                )
                ...
                w[y[config.hid - config.rank], np.arange(current_batch_size)] = (
                    -1 * config.delta
                )
                ...
                dW_dt = (
                    torch.mm(w, images)
                    - torch.sum(w * input_current, 1).unsqueeze(1) * synapses
                )
    ...
    if save:
        torch.save(
            synapses, f"models/synapses_hid_{config.hid}_batch_size_{config.batch_size}"
        )
